...
export class TeacherFormComponent extends BaseComponent implements OnInit {
    ...
    prepareForm(model?: Teacher): void {
        model = model || new Teacher();
        model.teacherId = model.teacherId || this.id || 0;
        ...
        this.form = this.formBuilder.group({
            ...
            subject: [model.subject || null, [FormValidators.required()]],
    ...
    saveFormData(isExit: boolean): void {
        ...
        this.subscriptions = this.service.create(this.form.value as Teacher).subscribe((next: Teacher) => {
            if (isExit) {
    ...
    updateFormData(isExit: boolean): void {
        ...
        this.subscriptions = this.service.update(this.form.value as Teacher).subscribe((next: Teacher) => {
            if (isExit) {
                this.router.navigate(['../../'], { relativeTo: this.route }).then();
            } else {
                this.resetFormData();
